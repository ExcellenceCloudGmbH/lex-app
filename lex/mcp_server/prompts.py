...
def register(server: FastMCP) -> None:
    @server.prompt(
        name="lex_investigate_calculation",
    ...
    def investigate_calculation(calculation_id: str) -> str:
        return (
            ...
            "Steps:\n"
            f"1. Call tool `lex_calculations_tail_logs` with `calculation_id={calculation_id}` "
            "and the matching `calculation_record` to inspect the live log buffer.\n"
            f"2. Call tool `lex_calculations_list_logs` with `calculation_id={calculation_id}` "
    ...
    def summarize_model(
        ...
        return (
            ...
            f"2. Call tool `lex_entries_list` with `model_container={model_container}`, "
    ...
    def audit_record(model_container: str, pk: str) -> str:
        return (
            ...
            f"2. Call tool `lex_entries_history` with `model_container={model_container}`, "
            f"`pk={pk}` for the simple_history timeline.\n"
            f"3. Call tool `lex_calculations_list_logs` (no filter) and identify any "
    ...
    def check_permissions(model_container: str) -> str:
        return (
            ...
            "1. Call tool `lex_permissions_user` for the principal's overall "
            "ra-rbac scopes.\n"
            f"2. Call tool `lex_permissions_model` with `model_container={model_container}` "
