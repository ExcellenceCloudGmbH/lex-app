...
class CalculationWaitContextTests(SimpleTestCase):
    ...
    def test_legacy_and_canonical_celery_task_modules_share_wait_context(self):
        ...
        import lex.lex_app.celery_tasks as legacy_celery_tasks
    ...
    def test_calculate_hook_dispatches_inside_worker_without_outer_wait_context(self):
        """Inside a worker with no outer context, the calc now dispatches via its
        own WaitForTasks (default-dispatch) instead of running inline. The
        worker-detection inline branch was removed deliberately."""
        instance = self._build_instance(3)
        task_result = Mock()
        task_result.id = "task-3"
        ...
        # Own WaitForTasks scope opened and drained on exit.
        task_result.get.assert_called_once_with()
