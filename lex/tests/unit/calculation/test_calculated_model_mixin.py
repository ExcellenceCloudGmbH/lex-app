...
class CalculatedModelMixinTests(SimpleTestCase):
    ...
    def test_dispatch_processing_dispatches_inside_worker_without_explicit_async_context(self):
        """Inside a worker with no explicit async context, dispatch now fans out
        to Celery by default (the worker-detection inline branch was removed).
        The CeleryTaskDispatcher opens its own WaitForTasks scope when none is
        active, so the parent still blocks on its children."""
