...
class TestShouldUseCelery(SimpleTestCase):
    ...
    def test_returns_true_even_when_func_has_no_delay(self):
        """``CELERY_ACTIVE=true`` → dispatch, even for undecorated methods.

        ``should_use_celery`` deliberately ignores whether ``lex_func()``
        exposes ``.delay``: an undecorated calculate/update method is
        dispatched through the generic ``calc_and_save`` task (see
        ``dispatch_calculation_task``), so every CalculationModel is
        Celery-eligible without requiring ``@lex_shared_task``.
        """
        obj = StubCalcModel()
        # StubCalcModel.calculate is a plain method, no .delay — still dispatches
        self.assertTrue(obj.should_use_celery())
