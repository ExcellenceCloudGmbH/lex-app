...
class GetSerializerClassTests(SimpleTestCase):
    ...
    def test_unknown_serializer_raises_api_exception(self):
        """Unknown serializer name → ValidationError (HTTP 400) with error + available keys.

        The lex_view callbacks contract (docs/features/access-and-ui/lex_view callbacks.md)
        specifies that an unresolvable ``?serializer=`` value must surface as a client
        error (400), not a server error (500). ValidationError carries
        ``status_code = 400`` and is still an APIException, so the existing
        ``isinstance`` checks downstream remain valid.
        """
        model_cls = _make_model_class()
        view = _build_mixin_view(
            model_class=model_cls,
        ...
        self.assertIn("Unknown serializer", str(detail["error"]))
        self.assertIn("nonexistent", str(detail["error"]))
