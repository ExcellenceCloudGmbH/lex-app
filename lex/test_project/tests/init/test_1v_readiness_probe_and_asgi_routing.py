...
Cluster 1v — scenarios 1.179–1.187. Type: U.
Covers: lex/lex_app/fast_health.py, lex/lex_app/asgi.py.
Run: python -m lex pytest lex/test_project/tests/init/test_1v_readiness_probe_and_asgi_routing.py -v
"""
...
class TestCluster01v_ReadinessProbeAndAsgiRouting(SimpleTestCase):
    """Cluster 1v: readiness probe path-matching, response shape, and ASGI routing."""

    # -- 1.187 ---------------------------------------------------------
    def test_1_179_is_readiness_path_accepts_all_canonical_forms(self) -> None:
        """
        Scenario 1.179: is_readiness_path() recognises every registered path.
    ...
    # -- 1.172 ---------------------------------------------------------
    def test_1_180_is_readiness_path_rejects_non_readiness_paths(self) -> None:
        """
        Scenario 1.180: is_readiness_path() returns False for non-readiness paths.
    ...
    # -- 1.173 ---------------------------------------------------------
    def test_1_181_readiness_asgi_app_returns_200_when_db_ready(self) -> None:
        """
        Scenario 1.181: readiness endpoint returns 200 and ready body when DB is up.
    ...
    # -- 1.174 ---------------------------------------------------------
    def test_1_182_readiness_asgi_app_returns_503_when_db_unavailable(self) -> None:
        """
        Scenario 1.182: readiness endpoint returns 503 when DB is unreachable.
    ...
    # -- 1.175 ---------------------------------------------------------
    def test_1_183_readiness_response_has_no_cache_and_json_content_type(self) -> None:
        """
        Scenario 1.183: readiness response carries no-store cache-control and JSON
        content-type header.
        Given: _database_ready() returns True
    ...
    # -- 1.176 ---------------------------------------------------------
    def test_1_184_database_ready_returns_false_on_exception(self) -> None:
        """
        Scenario 1.184: _database_ready() returns False when the DB cursor raises.
    ...
    # -- 1.177 ---------------------------------------------------------
    def test_1_185_http_application_routes_health_path_to_liveness_app(self) -> None:
        """
        Scenario 1.185: http_application() dispatches liveness paths to health_asgi_app.
    ...
    # -- 1.178 ---------------------------------------------------------
    def test_1_186_http_application_routes_readiness_path_to_readiness_app(self) -> None:
        """
        Scenario 1.186: http_application() dispatches readiness paths to readiness_asgi_app.
    ...
    # -- 1.187 ---------------------------------------------------------
    def test_1_187_http_application_routes_other_paths_to_django(self) -> None:
        """
        Scenario 1.187: http_application() falls through to django_asgi_app for
        ordinary paths.
