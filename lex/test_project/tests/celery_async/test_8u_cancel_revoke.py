"""Cluster 8u: cancellation-aware CallbackTask failure mapping.
...
Cluster 8u — scenarios 8.73–8.77. Type: U (SimpleTestCase — exercises
...
Run: python -m lex pytest lex/test_project/tests/celery_async/test_8u_cancel_revoke.py -v
"""
...
class TestCluster08u_CancellationExceptionDetector(SimpleTestCase):
    """Cluster 8u: ``_is_cancellation_exception`` mapping."""

    # ------------------------------------------------------------------
    # 8.73 — every documented cancellation class is recognised
    # ------------------------------------------------------------------
    def test_08_73_recognises_celery_and_billiard_cancellation_classes(self):
        """
        Scenario 8.73: every name in the cancellation allow-list is recognised.
    ...
    # ------------------------------------------------------------------
    # 8.74 — generic runtime errors are NOT cancellations
    # ------------------------------------------------------------------
    def test_08_74_generic_runtime_errors_are_not_cancellations(self):
        """
        Scenario 8.74: unrelated exceptions stay on the ERROR path.
    ...
    # ------------------------------------------------------------------
    # 8.75 — subclasses of cancellation classes are also recognised
    # ------------------------------------------------------------------
    def test_08_75_subclasses_of_cancellation_classes_are_recognised(self):
        """
        Scenario 8.75: the detector walks the MRO so subclasses count too.
    ...
    # ------------------------------------------------------------------
    # 8.76 — None / non-exception input is safe (returns False)
    # ------------------------------------------------------------------
    def test_08_76_none_returns_false_safely(self):
        """
        Scenario 8.76: defensive guard for the on_failure code path.
