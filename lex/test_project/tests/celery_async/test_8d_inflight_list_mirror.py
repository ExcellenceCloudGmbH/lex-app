...
class TestCluster08d_InflightListMirror(SimpleTestCase):
    ...
    def test_8_164_dispatch_claim_raises_the_signal_before_any_worker(self):
        """
        Scenario 8.164: the scale signal rises at dispatch, not at task start.
        Given: a task claimed at dispatch (worker pod still Pending)
        When: claim_dispatched() runs, then task_prerun's register() upgrades it
        Then: LLEN is 1 after the claim and STILL 1 after the upgrade — the
              supervisor comes up while the pod is queued (the window incident
              1410 left unguarded) and the upgrade never double-counts
        """
        registry.claim_dispatched("tid-1", "calc_and_save", (), {}, "celery")
        self.assertIn("tid-1", self._index())
        self.assertEqual(
            self._llen(), 1,
            "A dispatched-but-unstarted task must already show as in-flight, "
            "or the supervisor stays at 0 while the worker pod is Pending.",
        )

        registry.register("tid-1", "calc_and_save", (), {}, "celery")
        self.assertEqual(
            self._llen(), 1,
            "The claim->running upgrade re-SADDs the same id; it must not "
            "grow the list.",
        )
    ...
    def test_8_161_reconcile_keeps_ids_registered_mid_pass(self):
        """
        Scenario 8.161: reconcile never drops a concurrently-registered id.
        Given: a worker registers a task after reconcile has read the index
               but before it writes the list (the rebuild race)
        When: reconcile_inflight_list() runs against that interleaving
        Then: the new id survives — the mirror must not undercount, because
              KEDA would otherwise scale the supervisor away mid-calculation
        """
        registry.register("early", "calc_and_save", (), {}, "celery")

        real_smembers = self.fake.smembers
        seen = {"called": False}

        def racing_smembers(key):
            members = real_smembers(key)
            # First read only: a worker registers immediately afterwards, so
            # the snapshot reconcile is working from is already stale.
            if key == redis_keys.index_key() and not seen["called"]:
                seen["called"] = True
                registry.register("late", "calc_and_save", (), {}, "celery")
            return members

        with mock.patch.object(self.fake, "smembers", side_effect=racing_smembers):
            registry.reconcile_inflight_list()

        self.assertEqual(
            set(self.fake.lrange(redis_keys.inflight_list_key(), 0, -1)),
            {"early", "late"},
            "An id registered mid-reconcile must remain in the scale signal.",
        )

    def test_8_162_reconcile_collapses_duplicate_occurrences(self):
        """
        Scenario 8.162: LLEN stays an exact count of tracked tasks.
        Given: a list carrying the same tracked id twice (a crash between the
               SET and LIST writes, then a re-register)
        When: reconcile_inflight_list() runs
        Then: the duplicate collapses to one entry, so the pod's scale signal
              reflects real in-flight work rather than an inflated count
        """
        registry.register("dup", "calc_and_save", (), {}, "celery")
        self.fake.lpush(redis_keys.inflight_list_key(), "dup")
        self.assertEqual(self._llen(), 2, "Precondition: the list is inflated.")

        total = registry.reconcile_inflight_list()

        self.assertEqual(total, 1)
        self.assertEqual(
            self.fake.lrange(redis_keys.inflight_list_key(), 0, -1), ["dup"]
        )

    def test_8_163_supervisor_loop_reconciles_after_every_sweep(self):
        """
        Scenario 8.163: drift self-heals for the life of the pod, not just at boot.
        Given: the supervisor loop running one pass
        When: that pass completes
        Then: the in-flight list is reconciled afterwards — on-demand pods live
              across many calculations, so a stale entry reconciled only at
              startup would pin the pod up until it is restarted
        """
        registry.register("tracked", "calc_and_save", (), {}, "celery")
        self.fake.lpush(redis_keys.inflight_list_key(), "leaked-never-tracked")
        self.assertEqual(self._llen(), 2, "Precondition: the signal is stale.")

        def stop_after_first_pass(_app=None):
            supervisor.stop_forever()
            return {}

        with mock.patch.object(supervisor, "_get_app", return_value=mock.MagicMock()), \
             mock.patch.object(supervisor, "scan_and_recover", side_effect=stop_after_first_pass):
            supervisor.run_forever(interval=0)

        self.assertEqual(
            self.fake.lrange(redis_keys.inflight_list_key(), 0, -1), ["tracked"],
            "The loop must converge the signal onto the index after each sweep.",
        )
