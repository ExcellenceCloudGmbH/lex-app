...
# =====================================================================
# TestCluster04g_UserPermissionsView
#
# Drives the view directly with a fake request carrying
# ``user_permissions`` in the shape Keycloak delivers after the
# middleware has run. Asserts the documented ra-rbac output shape.
# =====================================================================
class TestCluster04g_UserPermissionsView(SimpleTestCase):
    """Scenarios 4.27 – 4.30 — ``UserPermissionsView.get``."""

    def _call_view(self, request):
        """Drive the DRF APIView through ``.as_view()`` so the response
        goes through the real renderer path."""
        view = UserPermissionsView.as_view()
        return view(request)

    def _fake_request(self, user_permissions=None, user=None, authenticated=True):
        """Build a DRF-compatible request with ``user_permissions``
        attached exactly the way :class:`KeycloakPermissionsMiddleware`
        would."""
        factory = RequestFactory()
        req = factory.get("/api/user-permissions/")
        # Force authenticated — the endpoint demands ``IsAuthenticated``.
        if user is None:
            user = MagicMock(is_authenticated=authenticated)
            user.profile = SimpleNamespace(uma_permissions=[])
        req.user = user
        if user_permissions is not None:
            req.user_permissions = user_permissions
        return req

    # -- 4.27 ----------------------------------------------------------
    def test_4_27_translates_scopes_into_ra_rbac_entries(self) -> None:
        """Scenario 4.27: each ``(rsname, scope)`` pair becomes one
        ra-rbac entry with ``action`` and ``resource`` keys.

        The frontend iterates this list to enable/disable buttons —
        one missed scope means the UI silently hides a feature the
        user *should* be able to use.
        """
        req = self._fake_request(
            user_permissions=[
                {"rsname": "lex_app.Invoice", "scopes": ["read", "edit"]},
                {"rsname": "lex_app.Payment", "scopes": ["read"]},
            ],
        )
        resp = self._call_view(req)

        self.assertEqual(resp.status_code, 200)
        data = resp.data
        self.assertEqual(
            sorted((e["resource"], e["action"]) for e in data),
            [
                ("invoice", "edit"),
                ("invoice", "read"),
                ("payment", "read"),
            ],
            "Every (resource, scope) pair must produce exactly one "
            "ra-rbac entry; resource must be the lowercase tail of "
            "the dotted rsname so the frontend can route on "
            "lowercase model names.",
        )

    # -- 4.28 ----------------------------------------------------------
    def test_4_28_missing_rsname_entries_are_skipped(self) -> None:
        """Scenario 4.28: a permission entry with no ``rsname`` is
        silently dropped — the frontend must never receive an entry
        it cannot map to a resource name.

        Keycloak occasionally emits bookkeeping permissions (e.g. the
        default ``Default Resource`` marker) with no ``rsname``;
        forwarding them would poison the ra-rbac dictionary.
        """
        req = self._fake_request(
            user_permissions=[
                {"scopes": ["read"]},  # no rsname — must be dropped
        ...
        self.assertEqual(resp.data[0]["resource"], "invoice")

    # -- 4.29 ----------------------------------------------------------
    def test_4_29_resource_set_id_becomes_record_field(self) -> None:
        """Scenario 4.29: a permission with ``resource_set_id``
        surfaces as ``record={"id": X}`` — this is how ra-rbac
        expresses *row-level* permissions.

        The ``with_instance`` Keycloak flow puts the actual resource
        id into ``resource_set_id``; a bug that drops this field
        collapses row-level auth into model-level auth and leaks data.
        """
        req = self._fake_request(
            user_permissions=[
                {
                    "rsname": "lex_app.Invoice",
                    "scopes": ["edit"],
                    "resource_set_id": "7",
                },
            ],
        ...
        self.assertEqual(entry["resource"], "invoice")
        self.assertEqual(
            entry.get("record"), {"id": "7"},
            "When Keycloak supplies a resource_set_id, the view must "
            "include a 'record' field so the frontend can scope the "
            "button to that specific row.",
        )

    # -- 4.30 ----------------------------------------------------------
    def test_4_30_falls_back_to_profile_uma_permissions(self) -> None:
        """Scenario 4.30: if the middleware did NOT attach
        ``request.user_permissions``, the view consults
        ``request.user.profile.uma_permissions`` instead.

        This is the non-Keycloak dev path — the profile carries the
        caller's stored UMA permissions and the view must honour it.
        """
        user = MagicMock(is_authenticated=True)
        user.profile = SimpleNamespace(
            uma_permissions=[{"rsname": "lex_app.Report", "scopes": ["read"]}],
        )
        req = self._fake_request(user_permissions=None, user=user)
        # Explicitly strip the attribute so the view hits the fallback.
        if hasattr(req, "user_permissions"):
            delattr(req, "user_permissions")

        resp = self._call_view(req)
        self.assertEqual(resp.status_code, 200)
        self.assertEqual(len(resp.data), 1)
        self.assertEqual(resp.data[0]["resource"], "report")
