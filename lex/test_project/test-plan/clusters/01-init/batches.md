...
| Property | Value |
| --- | --- |
| Scenario range | 1.223 – 1.262 |
| Type | U |
| Files covered | `lex/lex_app/streamlit/_client.py` (`resolve_api_base_url`, `_headers`, `get_json`, `patch_json`, `LexApiError`), `lex/lex_app/streamlit/calculation.py` (`read_status`, `read_status_cached`, `invalidate_status`, `status_cache_max_age`, `optimistic_status`, `trigger_calculation`, `poll_interval_for`, `poll_timer_needs_rebuild`, `presentation_for`, `last_run_caption`, `widget_key_for`, `lex_calculation`), `lex/lex_app/streamlit/__init__.py` |
| Test file | `lex/test_project/tests/init/test_1ab_calculation_widget.py` |
| Test classes | `TestCluster01ab_CalculationClient` (1.223–1.227 URL resolution, transport and HTTP failures, bearer auth), `TestCluster01ab_CalculationWidgetState` (1.228–1.240 poll lifecycle, presentation, trigger transport, design-token and no-Django-import gates), `TestCluster01ab_CalculationWidgetSurface` (1.241–1.250 full render against a fake Streamlit, including the disabled-with-reason button), `TestCluster01ab_CalculationSharedReads` (1.251–1.259 what a page costs — shared reads, the per-session cache boundary, optimistic click, and when the app may rerun), `TestCluster01ab_CalculationRerunBudget` (1.260–1.262 the same claims against the real Streamlit runtime, via `AppTest`) |
| Fixtures | none — `mock.patch` over `requests`, plus an in-file `_FakeStreamlit` standing in for `st` (fragment, columns, button, session_state, `rerun` as a raised sentinel). Its `button()` records the `disabled` it was drawn with and refuses to report a press for a disabled button, as the real host does — otherwise a scenario could "click" something the reader could not touch; it also keeps each declared fragment body so a scenario can invoke one on its own, which is what a poll tick is. An in-file `_Clock` substitutes for the widget's `time` module, so the cache's age bound can be crossed without sleeping. 1.260–1.262 use Streamlit's own `AppTest` instead: they are claims about how the runtime re-executes a script and how far `session_state` reaches, and a stand-in can be made to agree with any of them |
