...
| Property | Value |
| --- | --- |
| Scenario range | 1.223 – 1.275 |
| Type | U |
| Files covered | `lex/lex_app/streamlit/_client.py` (`resolve_api_base_url`, `_headers`, `_session`, `get_json`, `patch_json`, `LexApiError`), `lex/lex_app/streamlit/_status_poller.py` (`StatusState`, `read_status`, `trigger_calculation`, `StatusPoller.{set_token,watch,peek,is_pending,trigger_error,request_trigger,is_watching,run_once,stop}`, `get_poller`), `lex/lex_app/streamlit/calculation.py` (`presentation_for`, `poll_interval_for`, `ui_refresh_for`, `optimistic_status`, `last_run_caption`, `widget_key_for`, `lex_calculation`), `lex/lex_app/streamlit/__init__.py` |
| Test file | `lex/test_project/tests/init/test_1ab_calculation_widget.py` |
| Test classes | `TestCluster01ab_CalculationClient` (1.223–1.227, 1.263 — URL resolution, transport and HTTP failures, bearer auth, per-thread connection pooling), `TestCluster01ab_CalculationWidgetState` (1.228–1.240 poll lifecycle, the unconditional redraw timer, presentation, trigger transport, design-token and no-Django-import gates), `TestCluster01ab_CalculationWidgetSurface` (1.241–1.250 full render against a fake Streamlit and a real poller, including the disabled-with-reason button), `TestCluster01ab_CalculationPoller` (1.251–1.262 where the waiting happens — no I/O on the render path, shared watches, queued triggers, self-terminating and self-re-arming polls, the per-session boundary), `TestCluster01ab_CalculationRerunBudget` (1.264–1.266 the same claims against the real Streamlit runtime, via `AppTest`) |
| Fixtures | none — an in-file `_transport` patches `_client._session` (**not** `requests.get`: the client pools one Session per thread, so a scenario patching the module function would sail past it and out to the network); `_poller()` builds a **real** `StatusPoller` with `autostart=False` and its reader/trigger/clock injected, driven one pass at a time through `run_once()`, so no scenario starts a thread or sleeps; `_FakeStreamlit` stands in for `st` (fragment, columns, button, session_state, `rerun` as a raised sentinel), reports a press **once** as the real host does, and keeps each declared fragment body so a scenario can invoke one on its own — which is what a redraw tick is. `_Clock` moves the poller's deadlines without sleeping. 1.264–1.266 use Streamlit's own `AppTest`: they are claims about how the runtime re-executes a script and on which thread, and a stand-in can be made to agree with any of them |
| Tests landed | **53 pass / 0 fail** |
| Coverage gain | the whole Streamlit widget surface plus the new poller module |
| Prereqs | batch 10o (the status endpoint the widget reads, and the source of `can_calculate`) |
| Status | ✅ Complete — 53 pass / 0 fail |
| Note | The widget exists so a dashboard author can run one record without embedding a React table in an iframe to click one button. Three silent regressions are what the original scenarios gate. **Polling that never stops** — `poll_interval_for` returns `None` for every terminal status *and* for a failed read, so an idle dashboard is not permanent backend load multiplied by every tab left open (1.228–1.229), and the poller stops asking about a settled record while still re-arming when the reader triggers a run, which keeps stopping from being a one-way door (1.256, 1.262). **A failure path that raises** — Streamlit renders top-to-bottom, so an exception escaping a widget erases every widget below it: the page does not report a problem, it silently loses its bottom half. 1.232/1.236/1.244 drive refused, missing, broken and unreachable through a real render and assert a rendered message, and 1.266 drives a total outage through the real runtime and asserts all thirteen tiles still got their turn. **A second way to start a calculation** — 1.238 pins the import lists of *both* the widget and the poller so no Django model can creep in (an in-process ORM call would skip the record's read permission, resolve the wrong audit actor and miss `_defer_calculate_hook` in one step — the shape of the `edited_at` bug in PR #675), and 1.235 pins `calculate` into the PATCH *body*, since `One.update` reads `request.data` and never the query string, so a flag parked in the URL degrades into an empty partial update: a button that appears to do nothing. 1.237 keeps colours resolving from `design_system` (LEX success is teal, not green), and 1.231 keeps `ABORTED` rendering as interrupted-with-a-nudge rather than as a failure — the confusion that made incident 1410 hard to read. **1.247–1.250** close the disabled-with-reason gap without moving any decision into the client: the button is drawn disabled with the backend's own reason beside it when `can_calculate` is false (1.247), which is the server's answer carried in the envelope, not the widget's judgement. The other three keep the fallbacks honest, because the flag is an optimisation and never the authority — the double-trigger disable is independent of permission and still holds (1.248); a 403 arriving after the click is still rendered, since the envelope is a read old and only the PATCH is being authorised (1.249); and an envelope with no such key leaves the button alone, so a backend older than the flag does not disable it for everyone with no way through (1.250). The asymmetry is deliberate: enabled-then-403 costs a click, disabled-when-allowed is a dead end. **1.251–1.266 are the usability pass, and they replace an earlier one that did not work.** The first attempt cached reads and moved the timer decision earlier; a reader still reported tiles hanging before they rendered, a Calculate button that never looked interactive, and every other section greying whenever one calculation ran. The cache had been treating the symptom. Both remaining faults were the render path doing things a render path must not do, and the rule that ends them is one sentence: **the render performs no I/O and reruns nothing but itself.** *No I/O* — Streamlit runs a page on one thread, so a tile reading its own status inside the render holds up every tile below it, in series, before any of them exist; all reads and triggers moved to `StatusPoller`, a daemon thread the session owns, leaving the render a dictionary lookup (1.251 asserts zero reads during a thirteen-tile render; 1.264 asserts none on the script's own *thread*, which is the honest form of the claim once a poller exists; 1.252 keeps one record costing one read however many tiles show it; 1.253 pins that a click queues the PATCH rather than waiting for it, since `One.update` re-reads, saves, registers and broadcasts before answering). *No page reruns* — `run_every` is read only at declaration and only a full run declares, so any design that adapts the timer to the record's status must rerun the page to change it, and `st.rerun()` raises, taking every widget still to be drawn with it: one click cost fourteen script runs and 104 reads. The timer is now declared unconditionally and never rebuilt (1.230, 1.245, 1.246, 1.265). The one remaining rerun is deliberate and fires **once per record per session**, when the first read lands, so that `status = lex_calculation(...)` followed by a branch is true on a freshly opened page rather than only after the reader interacts; `sync_page=True` opts a single tile into following its record thereafter. That rerun is guarded by a call-stack flag rather than a `session_state` flag, because the poller fills snapshots on another thread and a record can go from unknown to known in the gap between an app-scope check and the fragment body — a page of tiles hits that gap constantly, and it reintroduced the exact cascade. **And a click waited to be told what it had just done**: `optimistic_status` renders "Running" from the accepted trigger — not a guess, since `One.update` saves `IN_PROGRESS` before answering — taking it back if refused (1.249) and yielding to the next real read (1.255). The per-session boundary is load-bearing and now covers a bearer token as well as status envelopes: the poller lives in `session_state`, never `cache_resource` or module state (1.261), and the polling thread never touches `st` at all (1.260), because a thread with no script run context that calls into Streamlit either raises or writes into whichever session happens to be current. Measured in a browser, 13 tiles over 6 records against a 300 ms backend: whole-page render seconds → **17 ms**, reads on the render thread 13 → **0**, click-to-"Running" ~7 s → **103 ms**, full script runs caused by a click 14 → **0**, page greyed seconds-at-a-time → **0.8 %** of wall clock in ~20 ms flickers. |