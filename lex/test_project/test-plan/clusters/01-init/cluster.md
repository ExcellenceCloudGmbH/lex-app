...
| 1.272 | The embed targets the record's own route | `show`, `edit` and `list` map to the React Admin routes the frontend already registers; an unknown view raises rather than framing a 404 page |
| 1.273 | The serializer reaches the frontend | the named serializer is handed to `lex_view`, so the field list lives on the model — the same name the REST API takes as a path segment |
| 1.274 | The embed hides the chrome by default | unlike `lex_view`: an iframe is a fixed box and the navigation is a large fraction of a 420px one |
| 1.275 | The embed adds no second way to reach the backend | its only first-party import is `lex_view` — a module that also spoke to the API would re-implement what embedding exists to avoid |

**Scenario range:** 1.223 – 1.275. **Test file:** `lex/test_project/tests/init/test_1ab_calculation_widget.py`. **Type:** U. **Status:** ✅ Complete (2026-08-05) — 53 pass / 0 fail. Sources: `lex/lex_app/streamlit/_client.py`, `lex/lex_app/streamlit/_status_poller.py`, `lex/lex_app/streamlit/calculation.py`, `lex/lex_app/streamlit/calculation_embed.py`, `lex/lex_app/streamlit/__init__.py`. Pairs with [batch 10o](../10-api_layer/batches.md), the status endpoint this reads — and, for 1.247–1.250, the source of the `can_calculate` flag the button is drawn from.
