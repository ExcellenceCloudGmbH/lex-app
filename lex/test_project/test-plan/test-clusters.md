...
### 7o. Scheduled calculations — `ScheduledCalculation.ensure` / `cancel` + worker entry point ✅

**Gap:** The published feature doc `docs/features/processing/scheduled-calculations.md` advertised a one-shot delayed-calculation API (`ScheduledCalculation.ensure(target=…, run_at=…, dedupe_tag=…)`) — Klaus enters provisions, the system schedules a single consolidated `Report` recalc for 18:00 tonight, every later provision joins the same scheduled run instead of triggering a new one. The implementation landed in `lex/lex_app/scheduling.py` + `lex/lex_app/celery_tasks.py::run_scheduled_calculation` + migration `0003_scheduledcalculation` but had **no paired tests**. 7o closes that gap end-to-end. Two design points the tests pin explicitly because they are *easy to forget*: (1) the dedupe contract is keyed on `(target_content_type, target_object_id, dedupe_tag, status="PENDING")` — second `ensure()` with the same tag is a *no-op* under the default `on_conflict="dedupe"`, the deadline does **not** move when more inputs arrive (this is the whole reason the feature exists; without it, every Provision save would push the report out by another 6 hours and Rebekka would never see it). (2) The worker (`run_scheduled_calculation`) deliberately **does not reimplement calculation dispatch** — it loads the target, calls `target.save()` with `is_calculated=IN_PROGRESS`, and the regular calc state machine + audit + history + cancel button take over from there. A regression that lost either of these would show up as silent data loss in the worst possible places (the report nobody runs because the schedule was clobbered, or a parallel calc pipeline that doesn't audit).

**Surface enumeration that drove scenario count.** `ensure()`: first-time, default-dedupe, replace, debounce-later/equal/earlier, missing/unsaved target, empty tag, bad `on_conflict`, run_at coercion (`timedelta` / aware dt / naive dt / bogus type), `CELERY_ACTIVE=false`. `cancel()`: PENDING happy path, terminal-state idempotency, None/unsaved safety. `run_scheduled_calculation`: happy path → schedule FIRED + target ran the pipeline through to SUCCESS, target deleted → MISSED with `error_message="target_deleted_before_fire"`, schedule row deleted → `"schedule_not_found"`, schedule already terminal → `"already_<status>"` no-op, target not a `CalculationModel` → MISSED with `error_message="target_is_not_a_calculation_model"`. Each is a distinct externally-observable behaviour and gets its own scenario.

**Bug found and fixed in the same change.** `_coerce_run_at` accepted only aware datetimes on input but stored them without normalising to the project's `USE_TZ`. Downstream `ensure()` then tried to compare the aware `run_at_dt` against a DB-roundtripped naive `existing.run_at` (`USE_TZ=False`, the customer-default), raising `TypeError: can't compare offset-naive and offset-aware datetimes` for *every* `on_conflict="debounce"` (and incidentally most equality assertions in 7o). Fixed by post-validation normalising the result: aware-on-input is still required (the documented contract — silently picking a tz is the "report ran 2 hours late" footgun), but if `settings.USE_TZ` is False the framework converts to naive in `TIME_ZONE` before storing, mirroring how Django itself converts on save to a `TIMESTAMP WITHOUT TIME ZONE` column. Tests in 7o use a `assertSameMoment` helper that compares POSIX timestamps so they work in both `USE_TZ` modes.

**Scenario range:** 7.176 – 7.192. **Test file:** `lex/test_project/tests/calculations/test_7o_scheduled_calculations.py`. **Type:** I. **Status:** ✅ Complete (Session 72 — June 3; 17 / 17 pass in 16.2s against PostgreSQL; the only mock is `CalculationModel.should_use_celery → False` for the worker-fires-pipeline scenario so the dispatched calc lands synchronously in the same process — the test environment has no broker).
