...
### Sub-cluster 8y — initial-data-upload calculations honour the normal dispatch contract

**Intent:** a `CalculationModel` seeded by the initial-data loader is a *root* calculation and must follow the **same** dispatch rules as a calc triggered from the UI/REST layer (PR #544 — "dispatch every root calculation to Celery, decorator-optional"). The loader (`ProcessAdminTestCase.setUpCloudStorage`) instantiates `klass(**parameters)` and `save()`s it inside an `OperationContext`; a seed object carrying `"is_calculated": "IN_PROGRESS"` (the documented arming contract in `docs/lex_topics/16-initial-data-upload.md`) makes that `save()` a calculation trigger → `CalculationModel.calculate_hook()`. So when `CELERY_ACTIVE=true` and the broker is reachable, the seeded calc is dispatched to a worker as a job — decorator-optional, through the generic `calc_and_save` task — exactly like a normal calc; otherwise it falls back to the same synchronous in-process execution.

**Branch parity:** `apps.async_ready` has two dispatch branches — the MQ/Worker branch wraps the load in `WaitForTasks()`, the legacy-threading branch does not. Both still dispatch each seeded root calc to Celery, because `calculate_hook` opens its own `WaitForTasks` when no outer scope exists. The only difference (legacy branch dispatches serially via per-calc scopes vs. the MQ/Worker branch's one shared scope) is a concurrency/profile difference, not a correctness gap — so **no source change**; this batch pins the dispatch contract against regression.

**Scenario range:** 8.116 – 8.120. **Test file:** `lex/test_project/tests/celery_async/test_8y_initial_data_calc_dispatch.py`. **Type:** I (`E2ETestCase`, `e2e_models=[CelerySyncCalc]`). **Status:** ✅ Complete (tests-only). 8.116 undecorated seed calc + reachable broker + no scope ⇒ dispatched via `calc_and_save`, not inline; 8.117 same under an explicit `WaitForTasks` scope ⇒ still dispatched; 8.118 `CELERY_ACTIVE` off ⇒ synchronous fallback, no dispatch; 8.119 broker unreachable ⇒ `should_use_celery` False ⇒ sync fallback; 8.120 seed object without `IN_PROGRESS` ⇒ not a trigger (no dispatch, no inline run, stays `NOT_CALCULATED`). Drives the real `setUpCloudStorage` over an in-memory seed plan (`get_test_data` is the file-IO boundary); mocks only the Celery decision boundary (`CELERY_ACTIVE` env + `celery.current_app.control.inspect`) and the `calc_and_save` / `execute_calculation_sync` seams. 5 pass / 0 fail locally; full `celery_async` cluster 121 pass / 4 skip.
