...
> * **BUG-001b expanded (Session 54).** 6.10 now has 6 companion scenarios: 6.10-control (sanity), 6.10b (AuditLogStatus child also wiped), 6.10c (3 retries → 0 rows), 6.10d (nested savepoint shape), 6.10e (real `calc.save()` inside request-scoped outer atomic), 6.10f (API POST → PATCH → fallback `calc.save()` inside outer atomic). All confirmed xfail.
