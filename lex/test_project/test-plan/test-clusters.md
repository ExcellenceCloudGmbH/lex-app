...
### 7o. Operator visibility + stuck-calculation recovery ✅

**Gap:** once the framework started tracking active calculations and exposing a live cancel path, operators still had no public way to answer the customer question **"what is still running, what is stuck, and can I recover it without SSH'ing into a worker?"** The intended surface is a read-only report of every active calculation (`list_in_progress()`), an age-filtered slice for triage (`find_stuck(older_than_seconds)`), and a bulk recovery entry point (`cancel_stuck(...)`) that reuses the existing per-record `cancel()` machinery instead of inventing a second cancellation path. The store-side contract that makes that possible is just as important: `mark_in_progress()` must stamp a stable start time on first entry, preserve it across re-entry so the visible age reflects when the calculation actually began, and `list_active()` must sort oldest-first and fail fast on nonsense thresholds instead of silently returning misleading data. A regression here is customer-visible immediately: an operator dashboard hides live work, age ordering lies, or the bulk-recovery sweep says "could not resolve instance" for calculations that are demonstrably still running.

**Scenario range:** 7.176 – 7.184. **Test file:** `lex/test_project/tests/calculations/test_7o_stuck_recovery.py`. **Type:** U. **Status:** ✅ Complete (Session 72 — June 2; 8 pure-logic scenarios pass and 7.184 is a strict xfail against **BUG-023** documenting that `cancel_stuck()` currently feeds on `find_stuck()`'s public projection, which strips `record_pk` before `_resolve_instance_for_entry()` runs, so bulk operator recovery misreports resolvable stuck rows as `could_not_resolve_instance` instead of reusing `cancel()` and collapsing descendants under their parent).
