...
**What it tests:** The `CalculationModel` state machine — transitions between NOT_CALCULATED, IN_PROGRESS, SUCCESS, ERROR, CANCELLED — across atomic and non-atomic models, and parent→child calculation hierarchies.
...
**Scenario range:** 8.73 – 8.77. **Test file:** `lex/test_project/tests/celery_async/test_8u_cancel_revoke.py`. **Type:** U. **Status:** ✅ Complete (Session 67 — June 1; extended Session 68 with 8.77 covering the audit-status mapping so CANCELLED is recorded as `failure`, not `success`; 5 pass / 12 sub-tests / 0.11s broker-free).
...
| 9.17 | `mark_in_progress` overwrites existing entry | Re-marking same id replaces prior entry (re-fire after CANCELLED reset) |
...
| 1.58b | `finalize_batch(failure_error=…)` collapses pending → failure with error string | Outer-driver exception path — audit trail closes out even for cancelled runs |
