...
| 8z. Initial-data seed calculations run on the dedicated calculation pool (backfilled) | 4 | 4 | 4 | 0 | 0 | ✅ Complete — scenarios 8.121–8.124; `ProcessAdminTestCase._save_seed_instance` offloads a seeded `IN_PROGRESS` `CalculationModel` to the `_calculation_executor` (`lex-calc`) pool like the live `One.update` path, but blocks on the future to preserve ordered seeding. Runs on the pool (not inline) / ordered-blocking / non-trigger skip / failure-propagation. Plan rows backfilled in the 8ab change |
| 8ab. Initial-data seed calculations add no per-calculation slowdown vs the request path (Session 87 — June 29) | 3 | 3 | 3 | 0 | 0 | ✅ Complete — scenarios 8.129–8.131; the seed-calc offload preserves the pool's concurrency budget (parked seed calc + concurrent probe on a different `lex-calc` worker), runs the body exactly once (no inline duplicate), and adds < 0.25s over bare pool execution of the same 0.30s body (bound below `SLEEP` catches a duplicate-/inline-run regression). Stable across 4 repeat runs |
...
| **Total** | **638** | **638** | **617** | **3** | **9** | Per-session narrative (what each session added, scenario renumberings, deferrals) lives in [`session-log.md`](session-log.md) — the single chronological record. |
