...
|---|---|---|---|---|---|
| 1. Init — Project Bootstrap | 28 | 237 | 131 | 0 | 0 |
...
| 1ab | MCP embed URL helpers + mode resolution + env-file I/O + asset verification | 1.223-1.237 | complete | 15 | 0 | 0 | Covers embed.py (_resolve_frontend_url, _csp_origins, _classify_path, _build_title, _build_embed_url), setup_with_ai.py (normalize_mcp_mode, normalize_ai_environments, update_env_file), verify_ai_assets.py (_read_env_file_value, _read_override_mode, resolve_active_mcp_mode, verify_directory), mcp_mode_invoke.py (_normalise_mode, InvokeSwitchResult.ok, invoke_switch_to_mode fallback). All unit tests, no DB required. |
