...
|---|---|---|---|---|---|
| 1. Init — Project Bootstrap | 28 | 262 | 156 | 0 | 0 |
...
| 1ab | Streamlit lex_calculation() widget — trigger one record and watch it | 1.223-1.262 | complete | 40 | 0 | 0 | native widget replacing "embed a whole React table just to click one button". Covers `lex/lex_app/streamlit/_client.py` (backend URL resolution, bearer token read from the host's `session_state["access_token"]`, `LexApiError`) and `calculation.py` (fragment poll loop, status presentation, trigger), plus the package `__init__` as the public surface (1.240). Gates the three properties a dashboard depends on. Polling self-terminates: `poll_interval_for` returns None on every terminal status and on a failed read, and the `st.fragment` `run_every` timer is rebuilt only when the interval actually changes, because only a full script run re-declares a fragment (1.228-1.230, 1.245-1.246). No failure path raises out of the widget - Streamlit renders top-to-bottom, so an escaping exception erases every widget below it (1.232, 1.236, 1.243-1.244). And the widget imports no Django model, so read permission, audit actor and the `_defer_calculate_hook` trigger path stay identical to the React UI's (1.238); `calculate` travels in the PATCH body, not the query string (1.235). 1.247-1.250 close the design's disabled-with-reason gap: the button is drawn disabled with the backend's reason beside it when the envelope's `can_calculate` is false (1.247), still disabled while a run is in progress for the separate double-trigger reason (1.248), still relays a 403 that arrives after the click because only the PATCH is authoritative and the envelope is one poll old (1.249), and left enabled when the key is absent, so a backend older than the flag does not disable everyone's button (1.250). Pairs with batch 10o, the status endpoint it polls. 1.251-1.262 are the usability pass after the first page built with thirteen of these widgets: a click took seven seconds to change a word, and running one calculation greyed every other widget on the page. Three causes, all measured against the real runtime with AppTest. Every widget issued its own blocking GET every run, so a page cost one round trip per widget in series - `read_status_cached` shares one read per (model, pk, include_log) for at most half a poll interval (1.251-1.253), out of `st.session_state` and never `st.cache_data`, because session_state is per-user and a status envelope describes a record the next reader may not be allowed to see at all - the endpoint 404s an unreadable record precisely so its existence is not disclosed (1.254, 1.262). The poll timer was decided inside the fragment, where the only way to obtain one is to rerun the whole script; `st.rerun()` raises, so every widget below lost its turn and asked for a rerun of its own next run. Reading status before declaring the fragment makes one pass enough (1.245 rewritten, 1.258), leaving app-scope reruns at exactly two per calculation - the click that starts the watch and the poll that ends it (1.259). And a click now renders "Running" from the trigger's own response rather than after a re-read (1.255), giving it back if the trigger was refused (1.256) and yielding to the next real read (1.257). Measured on 13 tiles over 2 records - load 13 GETs to 2, a rerun 13 to 0, a click 14 script runs and 104 GETs to 2 and 1 (1.260-1.261). |
