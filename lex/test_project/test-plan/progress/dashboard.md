...
| 7o. Scheduled calculations — `ScheduledCalculation.ensure` / `cancel` + `run_scheduled_calculation` worker (Session 72 — June 3) | 17 | 17 | 17 | 0 | 0 | ✅ Complete — scenarios 7.176–7.192; pins ensure() first-time / dedupe / replace / debounce, validation errors, `CELERY_ACTIVE=false` → `ScheduledCalculationUnavailable`, cancel() PENDING→CANCELLED + idempotency, worker happy-path FIRED + target deleted MISSED + schedule deleted not-found + already-terminal no-op + non-CalculationModel target MISSED. Surfaced + fixed `_coerce_run_at` `USE_TZ=False` aware-vs-naive comparison bug (debounce branch). |
