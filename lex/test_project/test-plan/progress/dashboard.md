...
| 1u. encrypted runtime metadata on health endpoint (Session 80) | 8 | 8 | 8 | 0 | 0 | ✅ Complete — scenarios 1.171–1.178; pins legacy status-only fallback, Fernet token derived from `LEX_API_KEY`, no plaintext version/SHA leakage, wrong-key failure, encryption-failure degradation, and both Django + fast ASGI health routes |
| 1v. Readiness probe + ASGI HTTP routing (Session 81 — June 18) | 9 | 9 | 9 | 0 | 0 | ✅ Complete — scenarios 1.179–1.187; `is_readiness_path()`, `readiness_asgi_app()` 200/503, `_database_ready()` exception→False, `http_application()` dispatch to health/readiness/django |
...
| 9f. Consumer disconnect cleanup — active-consumer set pruned on close (Session 81 — June 18) | 5 | 5 | 5 | 0 | 0 | ✅ Complete — scenarios 9.37–9.41; BackendHealthConsumer/CalculationLogConsumer/CalculationsConsumer all call discard() on disconnect; double-disconnect safe; disconnect_all() finds empty set |
