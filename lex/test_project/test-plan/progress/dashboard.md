...
| 8w. Cancelled-calculation audit terminal status (Session 75 — June 6) | 1 | 1 | 0 | 1 | 0 | 🚧 In progress — scenario 8.90 is an `@unittest.expectedFailure` repro for BUG-023 (API-triggered Celery cancellation leaves `AuditLogStatus` stuck at `pending` instead of terminal `cancelled`) |
