...
|---|---|---|---|---|---|
| 1. Init — Project Bootstrap | 28 | 275 | 169 | 0 | 0 |
...
| 1ab | Streamlit lex_calculation() widget — trigger one record and watch it | 1.223-1.275 | complete | 53 | 0 | 0 | native widget replacing "embed a whole React table just to click one button". Covers `lex/lex_app/streamlit/_client.py` (backend URL resolution, per-thread pooled session, bearer token read from the host's `session_state["access_token"]`, `LexApiError`), the new `_status_poller.py` (the session-owned thread that does every read and every trigger) and `calculation.py` (the render, status presentation, optimistic click), plus the package `__init__` as the public surface (1.240). Gates the three properties a dashboard depends on. Polling self-terminates: `poll_interval_for` returns None on every terminal status and on a failed read, and the poller stops asking about a settled record while re-arming when the reader triggers one (1.228-1.229, 1.256, 1.262). No failure path raises out of the widget - Streamlit renders top-to-bottom, so an escaping exception erases every widget below it (1.232, 1.236, 1.243-1.244, and 1.266 drives a total outage through the real runtime). And neither the widget nor the poller imports a Django model, so read permission, audit actor and the `_defer_calculate_hook` trigger path stay identical to the React UI's (1.238); `calculate` travels in the PATCH body, not the query string (1.235). 1.247-1.250 close the disabled- with-reason gap: the button is drawn disabled with the backend's reason beside it when the envelope's `can_calculate` is false (1.247), still disabled while a run is in progress for the separate double-trigger reason (1.248), still relays a 403 that arrives after the click because only the PATCH is authoritative and the envelope is a read old (1.249), and left enabled when the key is absent, so a backend older than the flag does not disable everyone's button (1.250). Pairs with batch 10o, the status endpoint it reads. 1.251-1.266 are the usability pass, and they replace an earlier one that did not work: after it, the reader still reported tiles hanging before they rendered, a Calculate button that never looked interactive, and every other section greying whenever one calculation ran. The earlier pass had cached reads and moved the timer decision earlier - it was treating a symptom. Both faults were the render path doing what a render path must not, and the rule that ends them is one sentence: the render performs no I/O and reruns nothing but itself. No I/O - Streamlit runs a page on one thread, so a tile reading its own status inside the render holds up every tile below it, in series, before any of them exist; all reads and triggers moved to a daemon thread the session owns, leaving the render a dictionary lookup (1.251-1.253, and 1.264 asserts no read lands on the script's own thread). No page reruns - `run_every` is heard only at declaration and only a full run declares, so adapting the timer to the status requires rerunning the page, and `st.rerun()` raises, taking every widget still to be drawn with it: one click cost fourteen script runs and 104 reads. The timer is now declared unconditionally and never rebuilt (1.230, 1.245-1.246, 1.265). One deliberate rerun remains, once per record per session when the first read lands, so that branching on the return value is true on a freshly opened page; `sync_page=True` opts one tile into following its record thereafter. It is guarded by a call-stack flag rather than session_state, because the poller fills snapshots on another thread and a record can go from unknown to known between an app-scope check and the fragment body - which reintroduced the exact cascade. A click renders "Running" from the accepted trigger rather than after a re-read (1.254), gives it back if refused (1.249) and yields to the next real read (1.255). The per-session boundary now covers a bearer token as well as status envelopes: the poller lives in session_state, never cache_resource or module state (1.261), and the polling thread never touches `st` at all (1.260). Measured in a browser, 13 tiles over 6 records against a 300ms backend: whole-page render seconds to 17ms, reads on the render thread 13 to 0, click-to-"Running" ~7s to 103ms, full script runs caused by a click 14 to 0, page greyed seconds-at-a-time to 0.8% of wall clock in ~20ms flickers. A third pass (1.267-1.271) fixed what the second one had made worse under a backend that is not perfectly healthy - and that backend is the normal case, because every request through KeycloakPermissionsMiddleware makes two uncached network calls to Keycloak, so a status read is a real round trip that sometimes does not come back. Two of the faults were introduced by the rewrite itself. A failed read set the watch inactive, so one dropped connection froze a tile on 'Status unavailable' for the rest of the session with a reload as the only way out; retries now back off from 1s to 30s, and only 403/404 stop a watch because only those are answers about the record (1.262). And liveness was refreshed by set_token alone, which only a full script run reaches - while the entire design is that a settled page never has one, so a calculation longer than the 90s idle timeout outlived the thread watching it and froze on 'Running'; peek() refreshes it now, and every redraw calls peek() (1.268). The rest is what made the page read as unreliable rather than actually be so: a failed read no longer erases a confirmed status, it keeps the badge and adds a 'Reconnecting...' caption, because the calculation did not stop being finished and 'Status unavailable' beside a visible status is both alarming and untrue (1.267); a trigger is dispatched on its own thread instead of queueing behind a read pass, which had it reaching the backend a second or more after the click while the badge already claimed the run had started - request_trigger now returns in ~1ms and the PATCH leaves within ~15ms (1.269); watches registered during one script run settle for 30ms so they leave in one parallel batch of up to eight, instead of the poller waking on the first and discovering the rest only after a full round trip (1.270); and the one page run the widget spends to announce a first answer is no longer spent on a failed read, which had left the return value stuck at None for the session (1.271). The placeholder before the first read is now an em dash rather than 'Checking', which thirteen tiles saying at once read as a page that could not reach its backend, and a click is acknowledged with 'Starting...' rather than 'Running' - a refusal a moment later is a story after the first and a contradiction after the second. 1.272-1.275 add the second way to put a calculation on a page, alongside the first rather than instead of it: lex_calculation() now embeds the lex-app frontend's own record view and the native widget keeps the same behaviour under the name lex_calculation_streamlit(). The embed is URL construction over lex_view and nothing else - 1.275 pins that its only first-party import is lex_view, because a module that also spoke to the API directly would be a second implementation of the exact thing embedding the product's own view exists to avoid. It targets the frontend's own per-record routes, which React Admin already registers for every lex resource (the app passes both list and show when it builds each Resource), so nothing invents a URL the app does not serve (1.272). The serializer name is the point of the signature: it is the same name the REST API takes as a path segment, /api/model_entries/<model>/<serializer>/one/<pk>, so declaring it on the model decides which fields appear with no frontend change and nothing listed twice (1.273). Chrome is hidden by default, unlike lex_view, because an iframe is a fixed box and the surrounding navigation is a large fraction of a 420px one (1.274). What the embed cannot do is return a value - a frame is a separate browsing context - so a dashboard that branches on a record's status still wants the native widget, which is why both are on the package surface and 1.240 now asserts both. |
...
| 10o | Read-only calculation-status endpoint for the Streamlit widget | 10.72-10.84 | complete | 13 | 0 | 0 | `GET /api/model_entries/<model:model_container>/<int:pk>/calculation-status` - the narrow read `lex_calculation()` polls, in `views/calculations/CalculationStatus.py`. Reads are filtered through `UserReadRestrictionFilterBackend`, the same backend list reads use, rather than a hand-rolled check - read permission here is a queryset filter, not a boolean. A record the caller may not read therefore returns the same 404 status AND the same body as one that does not exist (10.76); a distinguishable response would confirm the record exists and leak its calculation state. Terminal statuses stay distinct - ABORTED and CANCELLED are not collapsed into ERROR (10.73). The log tail is opt-in behind `?include_log=true`, so the 2-second poll never runs the `CalculationLog` query by default (10.77), is bounded to `LOG_TAIL_LIMIT=50` with an accurate `log_truncated` flag from one over-fetched row rather than a second COUNT (10.78-10.79), and is scoped - like the run timings, which the record itself does not carry - to the newest `calculationId`, so a re-run's tail carries none of the previous run's lines (10.80-10.81). 10.82-10.83 add `can_calculate` / `calculate_denied_reason`, the flag the widget draws its button from: read permission cannot answer "may this caller run it", so the endpoint instantiates `UserPermission` - the permission class `OneModelEntry` itself declares - and evaluates it against the trigger's own `{"calculate":"true"}` payload rather than re-deriving the rule. Both scenarios also issue the real PATCH, because a flag that disagrees with `One.update` lies in one direction or the other: enabled-then-403 is recoverable, disabled-when-allowed is a dead end. Pairs with batch 1ab, the widget that polls it. |
| 10p | Named serializers — choosing which fields a record shows | 10.85-10.88 | complete | 4 | 0 | 0 | named serializers - the mechanism that decides which fields a record shows, and what lex_calculation(serializer=...) passes through. api_serializers on the model class maps a name to a serializer class, and ?serializer=<name> selects between them in ModelEntryProviderMixin.get_serializer_class. The chain these scenarios protect crosses three codebases - a Streamlit helper that hands the name to lex_view, a React data provider that forwards it as meta.serializer, and this DRF view that resolves it - and only its two ends are visible to anyone: an author writes a name, a page shows fields. If the query parameter stopped selecting anything nothing would break; every dashboard would quietly show the default field list forever, which is the failure nobody notices. 10.85 pins the wide baseline the narrow view is measured against. 10.86 is the feature: a compact serializer declaring three fields returns three, and field_2/internal_note are absent rather than null. 10.87 keeps a typo loud - an unknown name is refused with the name and the available list in the body, because falling back to the default would render a plausible page showing the wrong fields. 10.88 pins that declaring a name adds to the map rather than replacing it: the React table and every framework-internal lookup read 'default', and a registration that displaced it would narrow the whole product to whatever one dashboard wanted. |
