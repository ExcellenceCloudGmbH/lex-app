...
| 8y. Initial-data-upload calculations honour the normal dispatch contract (Session 81 — June 11) | 5 | 5 | 5 | 0 | 0 | ✅ Complete — scenarios 8.116–8.120 (tests-only); a `CalculationModel` seeded by the initial-data loader routes through the same `calculate_hook` → `should_use_celery` → `dispatch_calculation_task` path as a UI/REST calc, so it dispatches as a Celery job (decorator-optional, via `calc_and_save`) when `CELERY_ACTIVE=true` + broker reachable in **both** `apps.async_ready` branches, and falls back to synchronous in-process execution otherwise. Drives the real `setUpCloudStorage` seed loader; mocks only the Celery decision boundary. No source change — pins existing behaviour against regression |
