...
def _pac_log(pac_checkout: Path):
    ...
    def run_log(from_ref: str | None, to_ref: str) -> str:
        ...
        result = subprocess.run(
            # Same format as digest._run_log, deliberately: a frontend
            # commit's body is the author's explanation just as much as a
            # backend one's, and asking for only %s left every frontend entry
            # as a bare subject line while backend entries carried real detail.
            ["git", "log", "--no-merges",
