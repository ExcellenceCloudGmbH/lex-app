...
\subsection*{CLI wrapper command}
The workflow is also exposed as a lex CLI management command and can be called from any path:
\begin{verbatim}
lex full_migration_workflow --db-name <DB_NAME> [--project-root <PATH>] [--v1-source <PATH>] ...
\end{verbatim}
When \texttt{--project-root} is omitted, the current working directory is used.
...
./lex/full_migration_workflow.sh <V1_MIGRATIONS_SOURCE> <V2_PROJECT_ROOT> <DB_NAME> \
  ...
  [--enable-sanitization] [--backfill-only] [--pre-clean-jsons] \
  [--rollback-on-failure] [--rollback-only] [--rollback-state-file <PATH>] \
...
\begin{itemize}[leftmargin=1.5em]
  ...
  \item \texttt{--backfill-only} runs only normalization + bitemporal backfill + audit-log backfill.
  \item \texttt{--pre-clean-jsons} removes workflow JSON artifacts before execution.
  \item \texttt{--rollback-on-failure} restores migrations to pre-run captured state.
  \item \texttt{--rollback-only} performs only rollback from captured migration-state file.
\end{itemize}
...
\begin{verbatim}
lex generate_legacy_freeze_manifest --before .lex_tables_before.json \
...
\subsection*{Operational details and invariants}
The manifest logic computes table diffs against current V2 model tables and known exclusions.
The invariant is that this pre-manifest reflects intended protected tables at the exact point
before new migration files are sanitized.
...
\end{verbatim}
This final manifest is consumed at runtime by legacy dynamic model registration.

\subsection*{Why this step is required}
The final freeze manifest is the contract between migration-time analysis and runtime
exposure. Without this artifact, legacy read-only table registration is either static
(incomplete) or heuristic (unsafe). The manifest makes legacy visibility explicit and
repeatable.

\subsection*{Operational details and invariants}
The command computes legacy table candidates by DB diff and model exclusions, then filters
invalid entries (for example, tables lacking a suitable primary key). The invariant is that
tables listed in this manifest are expected to exist post-migration and be safe for dynamic
unmanaged model registration.
...
lex backfill_audit_logging --chunk-size <N> --reason "V1 audit log migration snapshot" \
  [--dry-run]
\end{verbatim}
The workflow first normalizes legacy boolean-like \texttt{is\_calculated} values to
\texttt{SUCCESS}/\texttt{ERROR}, then backfills bitemporal rows, and finally backfills
audit logging rows from legacy archive tables.
