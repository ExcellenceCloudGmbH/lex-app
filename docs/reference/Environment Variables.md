...
| `REACT_APP_URL` / `LEX_FRONTEND_URL` | Origin of the React frontend. The auth proxy will accept a renewed dashboard token only from this origin (`POST /auth/adopt`), so leaving it unset closes that endpoint and an embedded dashboard stops renewing after its first token expires. Also used by `lex_view()` for the reverse direction. |
