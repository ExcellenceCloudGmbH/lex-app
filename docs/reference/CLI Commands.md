...
| `lex ai-worktree` | Prepare a second checkout so another LEX AI chat can work on the same repository in parallel |
...
`lex ai-worktree` is for when one chat is not enough. Two chats working the same repository share one working tree, so neither can tell which uncommitted line belongs to which run. This makes a git worktree, seeds it with the files git will not carry into one (`.env` and the project-scoped MCP configs, both deliberately untracked because they hold your token), and prints the folder to open the second chat in. The MCP config it writes there names the mode, so that chat starts on the right tool surface without switching. `--mode` picks it, `--name` describes the work and lands in the branch name, `--base` overrides the branch it cuts from -- by default the same one a run already live cut from, so the two diffs compose.
