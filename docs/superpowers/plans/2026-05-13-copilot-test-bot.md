...
- **The `lex test` venv path** in the PR-gate workflow's "run the test" steps mirrors the conventions.md instruction (`source ~/LUND_IT/ArmiraCashflowDB/.venv/bin/activate`). On GitHub-hosted runners that path won't exist — the `|| true` tolerates the absence. If the project uses a different runner setup, this step is the one to revisit.
