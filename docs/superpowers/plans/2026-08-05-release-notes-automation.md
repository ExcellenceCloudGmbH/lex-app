  ...
  pytest:
    ...
    steps:
      ...
      - name: Run tests
        ...
        # pytest walks upward from the test path and finds pyproject.toml on
        # its own, so its `-p no:django` addopt applies here too. That is
        # harmless: pytest-django is not installed on this runner, and
        # disabling a missing plugin is a no-op rather than an error.
