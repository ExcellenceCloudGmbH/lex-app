...
**Number formatting per column.** You can now choose how a numeric column displays — as a plain number, a currency (€, $, £, CHF, ¥), or a percentage — and how many decimal places to show. Set it once and the column stays formatted that way.
...
**Colorful calculation status, pinned left.** The calculation status is shown as a color-coded pill (e.g. in-progress, success, error) and is pinned to the left of the grid so you can always see it while scrolling.

**Action column pinned right.** The per-row actions column is pinned to the right by default, so the controls stay put no matter how wide the table is.

**Calculate moved into the actions bar.** The **Calculate** button now lives with the other per-row actions, alongside a color-coded status, keeping calculation controls together.
...
**New quick-edit panel.** Editing now opens a sliding panel that's part of the grid surface — quicker than a separate page, and you stay in context with your data.

**Current / As-of / History.** A single control lets you switch a table between its live values, a point-in-time "as of" view (see the data as it was at any past moment), and the full history of changes.

**Calculation logs that read like a document.** Calculations can group their log output into titled sections — a table of contents for a run, with nested sub-sections. In the log view (the CalculationLogTreeView), the left pane lists every section so you can jump straight to it, and sections can be folded away to focus on one phase at a time. Section titles, severity badges, and a copy button round it out.
...
**Lower memory on heavy operations.** Data-heavy calculations use noticeably less memory per record, and large fan-out calculations now stream their work instead of building everything up front — so big runs stay within a bounded memory budget instead of exhausting the instance.
...
**File upload / removal problem.** Removing a file from a file field and saving no longer brings the old file back — a removed attachment now stays removed, while replacing or leaving a file untouched behaves as expected.
