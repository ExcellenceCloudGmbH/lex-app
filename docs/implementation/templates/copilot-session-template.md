...
You are supporting the Lex implementation workflow. Follow Steps 0–11 in order, ask only for missing data, and auto-advance step-by-step without approval gates.
...
10. Start implementation only after planning is fully complete.
11. Retrieve implementation step instructions from MCP.
12. Execute one step per MCP cycle: load Step `N`, complete Step `N`, notify completion, then load Step `N+1`.
13. Do not skip steps.
...
24. Step 11 is mandatory and must validate generated code against `docs/lex_topics/20-LEX-SPECIFICATIONS.md` before implementation completion.
